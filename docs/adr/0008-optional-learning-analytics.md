...
O Hub registra por padrão eventos minimizados de início, checkpoint pela fronteira linear validada, tempo de reprodução, conclusão e falha técnica. O Aluno pode desativar essa análise em **Conta > Configurações**, sem modal, bloqueio ou tela dedicada. A preferência é oposição/opt-out, nunca consentimento.
...
O Admin seleciona um Curso e um período de 1, 3, 6 ou 12 meses. A tela mostra uma linha por Aula da publicação vigente, na ordem curricular; início, conclusão e falha somam todas as versões da mesma Aula, enquanto checkpoint e tempos de calendário usam os registros disponíveis das versões. O tempo reproduzido total aparece por Aula, soma a reprodução normal registrada no período e pode incluir reprises; ele não representa o tempo de calendário até concluir. O KPI de visualização média do Curso calcula a média, entre Matrículas ativas com analytics habilitado, da fronteira linear validada registrada em cada Aula ativa; conclusão vale 100% e ausência de registro vale 0%, inclusive quando o registro veio de uma versão anterior da mesma Aula. Conclusão manual ou automática por vídeo mantém sua origem quando conhecida. Matrículas ativas continuam sendo uma fotografia atual do Curso, sem soma entre versões. Versões históricas ficam acessíveis nos detalhes, e a exportação CSV fica limitada ao Curso e período selecionados, sem Conta, Matrícula, e-mail ou outro identificador pessoal. O Hub não mantém lista de Alunos inativos, registro de reengajamento, ação de contato nem automação de mensagens baseada nesses dados.
