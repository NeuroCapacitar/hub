...
Atualização de 8 de setembro: `find-docs` foi reduzida a um roteador de consulta e não oferece instalação global só para ler documentação. As cópias ativa e auxiliar foram alinhadas. As fontes `.codex/.tmp/plugins/plugins` de Superpowers, Vercel e HyperFrames foram atualizadas para não reintroduzir gates universais, reviews obrigatórios, AI Elements/gateway/streaming obrigatórios ou defaults visuais rígidos. Cópias de backup histórico não foram reescritas.

Também foram corrigidas as referências relativas que apontavam para a raiz errada da compilação React (`rules/async-defer-await.md`, `rules/async-cheap-condition-before-await.md`, `rules/server-hoist-static-io.md`), os links de exemplo inexistentes em `skill-creator`, os links placeholder de Wayfinder, os exemplos de saída de Domain Modeling e a tabela de referências ausente do Revenue-Centric importado. O link do setup de módulos continua deliberadamente apontando para o arquivo que o próprio workflow cria.

Validação atual: varredura em nove raízes de instruções, incluindo skills pessoais, `.codex` e fontes de plugins, encontrou zero ocorrências dos 20 padrões de gate/default/provedor listados no relatório. Varredura de todos os AGENTS.md nas raízes atuais encontrou zero links relativos quebrados e nenhum ponteiro `aidd_docs` no Hub ativo; `aidd_docs` permanece apenas no Hub Imports, onde o diretório existe e a memória é carregada por relevância. YAML das skills alteradas continua válido e `bun run docs:check` passou.
...
As pendências listadas para a próxima passagem foram processadas: contradição useEffectEvent, junctions ausentes, fontes de plugin, frontmatter, referências transitivas, roteamento UI, regras de revisão/TDD e gates de workflow foram verificados ou corrigidos. Permanecem fora do escopo somente históricos/backsups e fontes de terceiros não ativas; preservá-los evita destruir rastreabilidade.

Referências React: Context7 consultado com library React e docs /reactjs/react.dev. A documentação oficial https://react.dev/reference/react/useEffectEvent confirma identidade instável e uso apenas em Effects/Effect Events. Compilações AGENTS corrigidas no parágrafo, títulos e âncoras; exemplo de busca condicionado à semântica não reativa de onSearch.

Conferência adicional: 11 arquivos advanced-event-handler-refs.md corrigidos e registrados no CSV. Varredura dos diretórios React auditados encontrou zero ocorrências restantes da afirmação literal falsa sobre identidade estável. Frontmatter de todos os SKILL.md ativos verificados com o parser YAML após normalização de CRLF; 288 válidos, zero inválidos. Isso valida sintaxe, não a correção das APIs descritas.
...
Sites/references/environment.md: alinhado o passo de open_in_codex à permissão de URLs locais; ausência de handoff não bloqueia implementação. Backup e hash registrados, trecho final conferido.

Fontes sincronizadas e cópias importadas: Superpowers, Vercel, Neon, Sites, Cloudflare, Sentry e fontes `.codex/.tmp/plugins/plugins` foram alinhadas quando continham os mesmos gates ou defaults. Backups históricos e caches de terceiros foram preservados quando não eram a fonte ativa. O estado atual e hashes estão em `2026-09-08-instruction-state.csv`; o residual scan em `2026-09-08-residual-scan.txt` registra zero padrões-alvo.

Limites verificados: nenhuma instalação, commit, push, deploy ou alteração no produto foi executada. As mudanças são instruções locais, backups e documentação de auditoria. `bun run docs:check` é o check aplicável; exemplos de API e comportamentos de terceiros não foram executados.

Fechamento técnico da passagem atual: os códigos-fonte de plugin `.codex/.tmp` foram alinhados, incluindo Superpowers, Vercel e HyperFrames; as referências de AI SDK, Sites, Cloudflare, Domain Modeling, Wayfinder, Revenue-Centric e skill-creator foram ajustadas para não apontarem para arquivos ausentes nem impor defaults globais. O verificador de links relativos encontrou `broken=0` após excluir somente URLs externas e o arquivo de saída que o próprio workflow `setup-ts-deep-modules` cria. Evidência: `2026-09-08-link-validation.json`.

O residual scan atual cobre nove raízes de instruções e vinte padrões problemáticos, com `residualCount=0`; o estado de hashes está em `2026-09-08-instruction-state.csv`. Skills, AGENTS e caches históricos que não são fontes ativas permanecem preservados para rastreabilidade. Nenhuma alteração de produto, commit, push, instalação ou deploy foi feita.

Frontmatter atual: 288 SKILL.md nas raízes ativas foram validados pelo parser YAML, com `invalid=0`. Evidência: `2026-09-08-skill-frontmatter.json`.

Idempotência: `remediate-instructions.ps1` reaplicado após as últimas correções retornou `pending=0`; nenhum ajuste pendente no lote automatizado. A busca final continua com `residualCount=0`.

Conclusão da remediação: todos os achados acionáveis do catálogo ativo e das fontes sincronizadas foram corrigidos ou delimitados por intenção, risco e host. Histórico de terceiros, backups e snapshots foram preservados deliberadamente e não são instruções ativas. Não há alteração de produto pendente nesta tarefa.

Correções finais desta passagem: fontes sincronizadas `.codex/.tmp` receberam as mesmas regras opcionais de brainstorming, planos, TDD, revisão, diagnóstico, AI SDK, Vercel e HyperFrames; `poteto-mode` deixou de exigir checklist/citação ritual; `find-docs` foi reduzida e as descrições UI foram separadas por intenção. A busca de padrões ativos continua em zero após essas mudanças.
