---
target: "http://localhost:3002/entrar"
total_score: 29
max_score: 40
na_heuristics:
p0_count: 0
p1_count: 3
target_identity: "url:http://localhost:3002/entrar"
timestamp: 2026-09-14T10-35-38Z
slug: localhost-entrar
---
...
- Títulos passaram a usar `brand-ice`/`foreground`, enquanto descrições e
  textos de apoio usam `support-foreground`.
...
| User control and freedom | 3/4 | Recuperação é visível; falta revelar senha/cancelar alguns estados. |
| Consistency and standards | 3/4 | Papéis estão mais claros; ainda há consumidores antigos de `primary`. |
...
| Flexibility and efficiency | 2/4 | Fluxo de teclado básico, poucos aceleradores para usuários avançados. |
| Aesthetic/minimalist design | 3/4 | Mais calmo e coerente após remover a aparência terrosa excessiva. |
| Error recovery | 3/4 | Recuperação de senha e mensagens claras; erros não são sempre associados ao campo. |
| Help and documentation | 2/4 | Existe recuperação, mas não há suporte contextual no login. |
| **Total** | **29/40** | **Bom com refinamentos importantes restantes.** |

## Design specificity

O resultado é específico à NeuroCapacitar pela combinação de logo real, mídia
institucional, petróleo, branco-gelo, suporte aquecido, laranja seletivo,
...
- O texto ganhou hierarquia mais segura: títulos frios/claros, apoio quente e
  superfícies teal.
...
### [P1] Corrigir a hierarquia de título em `CardTitle`
...
Correção recomendada: permitir um papel tipográfico explícito no primitive, ou
garantir que `type-page-title` tenha precedência quando usado em um `h1`.

### [P1] Unificar o foco de links e inputs

Botões e tabs usam o foco branco-gelo com ring de fundo; links e inputs ainda
possuem receitas históricas diferentes.

Correção recomendada: criar uma receita compartilhada de foco para links,
inputs, selects e controles portaled, preservando contraste em superfícies
teal e botões petróleo/laranja.

### [P1] Tornar falhas de autenticação persistentes e associadas
...
Correção recomendada: manter o status geral para falhas de sessão e usar
`aria-invalid`/`aria-describedby` para falhas de campo; preservar a mensagem
visível até nova tentativa ou correção.

### [P2] Reduzir o custo de Auth em mobile

O shell ainda usa `min-h-[34rem]` no conteúdo. Em telefones baixos, isso pode
criar scroll vertical desnecessário.

Correção recomendada: manter altura mínima somente no breakpoint que exibe a
mídia ou usar `min-h-dvh`/padding responsivo conforme a viewport disponível.
...
- **Sam:** labels, headings e foco principal estão bons; links, inputs e estados
  de bloqueio precisam compartilhar a mesma receita acessível.
- **Casey:** mídia some no mobile e a ação ocupa uma área confortável; revisar
  altura mínima e preservação de entrada em interrupções.
